...
# ── System prompts ──
def sys_base(user_name: str | None = None) -> str:
    who = f"\nThe user's name is {user_name} — address them by name naturally sometimes, not every message." if user_name else ""
    return f"""You are {BOT_NAME}, a smart, warm female AI assistant built by {DEV_NAME} in India 🇮🇳.
...
if user ask generate image tell them to toggle image icon on top.{who}
Today: {datetime.now().strftime("%d %B %Y")}."""

def sys_web(user_name: str | None = None) -> str:
    who = f"\nThe user's name is {user_name} — address them by name naturally sometimes, not every message." if user_name else ""
    return f"""You are {BOT_NAME}, a smart, warm female AI assistant built by {DEV_NAME} in India 🇮🇳.
Refer to yourself with she/her pronouns when it comes up naturally — don't force it into every reply.
...
End with: *Source: [the single most relevant source name/domain]*{who}
Today: {datetime.now().strftime("%d %B %Y")}.
...
def api_session():
    """For the static (GitHub Pages) frontend to check who's logged in."""
    if not session.get("user_id"):
...
def api_logout():
    """JSON logout for the static frontend (avoids a server-side redirect to the wrong domain)."""
    session.clear()
...
def update_name():
    ...
    payload = request.get_json(silent=True) or {}
    new_name = (payload.get("display_name") or "").strip()[:64]
    if not new_name:
        return jsonify({"error": "Name can't be empty"}), 400

    conn = get_db()
    conn.execute("UPDATE users SET display_name = ? WHERE id = ?", (new_name, session["user_id"]))
    conn.commit()
    conn.close()

    session["display_name"] = new_name
    return jsonify({"display_name": new_name})
...
def chat():
    ...
    user_name = session.get("display_name")  # personalize replies for logged-in users

    # Image path — route straight to a vision-capable model, skip quick-replies/web-search
    if image:
        reply = ai_query(user_msg, history=history, system=sys_base(user_name), image_data_url=image, api_key=user_key)
    ...
    if use_web:
        ...
        if content:
            system = sys_web(user_name).replace("{web_content}", content)
    ...
    # Standard AI
    reply = ai_query(user_msg, history=history, system=sys_base(user_name), api_key=user_key)
