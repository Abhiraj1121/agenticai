document.addEventListener('DOMContentLoaded', () => {
  ...
  // on every /api/chat call; backend falls back to its own server key if absent.
  // ══════════════════════════════
  ...
  async function runAvatarTool(seed, style = 'bottts') {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tool/generate_avatar`, {
        method: 'POST',
        credentials: 'include', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
  ...
  async function runDocumentTool(topic, docType = 'report', format = 'md') {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tool/generate_document`, {
        method: 'POST',
        credentials: 'include', headers: { 'Content-Type': 'application/json', ...authHeaders() }, credentials: 'include',
  ...
  // ══════════════════════════════
  // AUTH GATE — this page is static (GitHub Pages), so login state must be
  // checked against the Render API on load. No session = bounce to /login.
  // ══════════════════════════════
  (async function checkAuth() {
    try {
      ...
      nameLabelInit(data.display_name || 'Account');
    } catch {
      window.location.href = `${API_BASE}/login`;
    }
  })();
  function nameLabelInit(name) {
  ...
  document.getElementById('logoutLink')?.addEventListener('click', async (e) => {
    ...
    finally { window.location.href = `${API_BASE}/login`; }
  });
  ...
  editNameBtn?.addEventListener('click', (e) => {
    e.stopPropagation();
    editNameInput.value = nameLabel.textContent.trim();
  ...
  async function saveName() {
    ...
    if (!newName || newName === nameLabel.textContent.trim()) return;
    try {
      ...
      const data = await res.json();
      if (res.ok) nameLabel.textContent = data.display_name;
    } catch (err) { console.error('Failed to update name', err); }
  }
  editNameInput?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') saveName();
    if (e.key === 'Escape') editNameInput.style.display = 'none';
  });
